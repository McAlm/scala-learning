...
class ListsSuite extends FunSuite {
  ...
  test("max of more larger and mixed numbers") {
    assert(max(List(3, 7, 37, 148, 6, 24789, 2)) === 24789)
  }
  
  test("max of some negative numbers") {
    assert(max(List(-3, -7, -37)) === -3)
  }
  ...
  test ("sum of some larger numbers"){
    assert(sum(List(12,25,37)) === 74)
  }
  
  test ("sum with negative numbers"){
    assert(sum(List(12,25,-37, 37)) === 37)
  }
}
